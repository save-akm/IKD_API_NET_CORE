...
{
    public class D3DVZNERepository
    {
        ...
        public IEnumerable<D3DVZNE> GetAll()
        {
            using (IDbConnection dbConnection = Connection)
            {
                
                string sQuery = @"SELECT * FROM DC2LIB.D3DVZNE ORDER BY DVZDSC ASC";
                dbConnection.Open();
                var result = dbConnection.Query<D3DVZNE>(sQuery);
                dbConnection.Close();
                return result;
            }
        }
